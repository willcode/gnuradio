      ...
      d_iir_valid(false)
{
    ...
    d_fft = std::make_unique<gr::fft::fft_complex_fwd>(fft_size);
                                         ...
                                         gr_vector_void_star& output_items)
{
    ...
    int n_wf = 0;
    int i;
    const float scale_factor = 1.0f * 1.0f / static_cast<float>(d_fft_size * d_fft_size);
    while (n < n_in) {
        ...
        // Update IIR and max
        if (!d_iir_valid) {
            memcpy(d_iir.data(), d_logfft.data(), d_fft_size * sizeof(float));
            memcpy(d_maxhold_buf.data(), d_logfft.data(), d_fft_size * sizeof(float));
            d_iir_valid = true;
        }

        for (i = 0; i < d_fft_size; ++i) {
            d_iir[i] = (1.0f - d_alpha) * d_iir[i] + d_alpha * d_logfft[i];
            d_maxhold_buf[i] = 0.001f * i; //std::max(d_maxhold_buf[i] * d_epsilon, d_logfft[i]);
        }
        ...
        if (d_histo_count == d_histo_decim) {
            ...
            for (i = 0; i < d_fft_size; i++) {
                d_logfft_b[i] = static_cast<uint8_t>(
                    std::clamp(lrintf(d_logfft[i] * convert_mul + convert_add), 0L, 255L));
            }
            ...
            for (i = 0; i < d_fft_size * d_num_bins; ++i) {
                out_histo[i] = static_cast<uint8_t>(
                    std::clamp(lrintf(d_histo_buf_f[i] * 256.0f), 0L, 255L));
            }
            ...
            for (i = 0; i < d_fft_size; ++i) {
                out_histo[i] = static_cast<uint8_t>(
                    std::clamp(lrintf(d_maxhold_buf[i] * convert_mul + convert_add), 0L, 255L));
            }
            ...
            for (i = 0; i < d_fft_size; ++i) {
                out_histo[i] = static_cast<uint8_t>(
                    std::clamp(lrintf(d_iir[i] * convert_mul + convert_add), 0L, 255L));
            }
            out_histo += d_fft_size;
        ...
        if (d_waterfall_count == d_waterfall_decim) {
            for (i = 0; i < d_fft_size; ++i) {
                out_waterfall[i] = static_cast<uint8_t>(
                    std::clamp(lrintf(d_logfft[i] * convert_mul + convert_add), 0L, 255L));
            }
            d_waterfall_count = 0;
            ++n_wf;
